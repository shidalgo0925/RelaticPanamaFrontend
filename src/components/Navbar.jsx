...
const Navbar = () => {
  ...
  const activities = [
    ...
    { name: 'Blog', href: 'https://relaticpanama.org/_blog/', icon: Newspaper }
  ];
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-200 shadow-sm">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-2 left-4 sm:left-10 w-6 h-6 sm:w-8 sm:h-8 border border-slate-300/30 transform rotate-45 opacity-20" />
        <div className="absolute top-4 right-8 sm:right-20 w-4 h-4 sm:w-6 sm:h-6 border border-slate-300/30 rounded-full opacity-20" />
        <div className="absolute bottom-2 right-4 sm:right-10 w-3 h-3 sm:w-4 sm:h-4 bg-slate-300/20 transform rotate-12 opacity-20" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 overflow-visible">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-shrink-0">
            <a href="#inicio" onClick={(e) => scrollToSection('inicio', e)} className="min-w-0 block flex items-center space-x-2 sm:space-x-3 cursor-pointer">
              <div className="relative flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12">
                <img src="/logo.png" alt="RELATIC Panamá" className="w-full h-full object-contain" />
              </div>
              <div className="hidden sm:block">
                <h1 className="text-sm sm:text-base lg:text-xl font-bold text-slate-800">
                  RELATIC PANAMÁ
                </h1>
                <span className="text-xs sm:text-xs lg:text-xs tracking-wider leading-tight block text-slate-600">
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-8 2xl:gap-10 flex-nowrap overflow-visible">
            <a 
              href="#inicio" 
              onClick={(e) => scrollToSection('inicio', e)}
              className={`px-2 py-1 ${textColor} ${hoverTextColor} transition-all duration-300 font-semibold relative group text-xs lg:text-sm xl:text-base 2xl:text-base cursor-pointer whitespace-nowrap`}
            ...
            {/* Servicios: enlace a página de servicios */}
            {!isSimplifiedMenu && (
              <a
                href="/servicios"
                className={`px-2 py-1 ${textColor} ${hoverTextColor} transition-all duration-300 font-semibold relative group text-xs lg:text-sm xl:text-base 2xl:text-base cursor-pointer whitespace-nowrap`}
            ...
            {!isSimplifiedMenu && (
              <div className="relative">
                <button
                  ...
                  className={`px-2 py-1 flex items-center gap-1 ${textColor} ${hoverTextColor} transition-all duration-300 font-semibold relative group text-xs lg:text-sm xl:text-base 2xl:text-base cursor-pointer whitespace-nowrap`}
                  style={{ fontFamily: 'Inter, sans-serif', letterSpacing: '0.01em' }}
                >
